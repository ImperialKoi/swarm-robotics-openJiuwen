...
- 48 sectors: rows `A..F` (y) × cols `1..8` (x), each **40 × 36 m**. Physical robot dimensions and sensor ranges are unchanged. The demo's `robot_speed_multiplier: 1.20` raises every robot's translation speed by 20%, including loaded carriers and aircraft; terrain, payload and flight multipliers still apply. Other scenarios default to 1.0. Turning limits and the simulation clock are unchanged. This is a pacing adjustment toward the owner's 180 s rescue target, not a measured guarantee of that target.
