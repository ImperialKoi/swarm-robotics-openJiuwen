...
cut scope and remaining operator preparation. Existing component defaults remain intact.
...
> **Scenario numbers in §1–§3 were corrected to match `assets/scenarios/demo.yaml`, which is the only source of truth.** They had drifted twice (map 320×208 → 480×320, casualties 8 → 80 → 120) and the document did not follow. That drift is not cosmetic: §6's 75% rescue target was set against the *first* of those scenarios and was still being quoted against the third. See §6.1.
...
| RL | MAP-Elites **local** (8 M1 cores). CV detector + GRPO ladder on **Kaggle GPU**. MAPPO stretch. **Unit policy (§7.8): behaviour cloning → PPO on Kaggle CPU sessions**, never on the laptop |
...
Target 5:30–6:30. Timings are sim-clock; the run is deterministic under a fixed seed, so these hold on rehearsal.

| t | Beat | What the operator does | What must be on screen |
|---|---|---|---|
| 0:00 | **Cold open** | Nothing. Eagle-eye, fog on, sectors on. | Fully fogged 48-sector map, 512 robots massed at base, HUD `0/80`. |
| 0:06 | **First directive** | Nothing. | Reasoning feed prints the hivemind's opening explore priorities, with a `source: base-local` badge — **not** `tuned-local`; no model was trained, and `mission.py` correctly leaves `tuned=False`. Auto-pan pulses the prioritized sectors. |
| 0:20–2:00 | **Exploration** | Nothing. | Fog peels back sector by sector. Ticker fires constantly with Tier-2 `last_action_reason` lines. Victims start logging. Relay chain visibly extends; a robot that outruns the chain greys out and reconnects. |
| 1:30 | **Follow-cam** | Click a scout. | Camera follows it; side panel shows its live `last_action_reason`. Talk over this: "this is Tier 2 — no LLM in this loop." |
| 2:00 | **God view** | Toggle god-view for ~8 s, then back. | True victim pins and true hazard extent appear next to what the swarm actually knows. *This is the "the hivemind doesn't cheat" beat.* |
| 1:30 | **Hazard ignites** | Nothing. | Hazard blooms at C4 and starts drifting. *(Measured. This row said 2:30 until D13; `demo.yaml` has said `ignite_t: 90` since D5d — [M-46](MEASUREMENTS.md).)* |
| 3:00 | **Reprioritize** | Nothing. | Hivemind reasoning feed explicitly abandons the threatened sector. Robots inside it get `retreat` tasks and visibly leave. Auto-pan snaps to that sector. *This is the "watch the AI think" beat.* |
| ~3:20 | **Failure** | Nothing (state-triggered). | Fire/smoke particle burst + screen shake on the doomed robot. HUD active-count ticks down. Within 1–3 s the event log shows `task_orphaned` → `task_awarded` to a different robot, and the ticker calls it out. *This is the "watch it self-heal" beat.* |
| 4:00 | **Hivemind offline** | Press `H`, count to ten, press `H` again. | Reasoning feed flips to a red `HIVEMIND OFFLINE` banner. Swarm keeps bidding, rescuing, self-healing. **Hold for ~10 s, no longer** - the point lands immediately and dead air costs more than the beat is worth. One narration line: "Tier 2 has no LLM in it." |
| 4:00–6:00 | **Endgame** | Nothing. | Remaining victims found, dug out, carried in. HUD climbs to final. |
| ~6:00 | **Close** | Nothing. | Final scorecard overlay. |
...
> **Status: never run.** This section is the proposed plan and is kept for the reasoning. No rung was climbed — `swarmmind/training/llm/` has never contained a file. The ladder sits at cut lines 5-7 in §7.1, the gate ships the scripted rung, and the CV detector was prioritized instead (a smaller, self-contained job). Do not describe the hivemind as tuned.
...
**The plan: climb the ladder, banking each rung.** Evaluate RAFT first, then attempt DPO and GRPO upgrades. Each is evaluated against the same held-out gate (§6); whichever wins ships. This is the difference between "we fine-tuned it" as a fact and as a hope.
...
Checked independently per component. **On the four demo maps, seeds 42–45**
...
| Whole system — **mission quality** (§6.1) | On demo seeds 42–45, Tier 3 off: **delivery ≥ 55%** of casualties *found* are rescued, **and discovery ≥ 55 of 110** on seed 42 | do not ship the change. |
...
### 6.1 Why the 75% rescue target was replaced
...
> **Scenario update.** The scenario was rescaled — 360 × 240 m, 512 robots, 80 casualties —
...
|---|---|
| **M1 — autonomous rescue** | Real perception, capability-gated auctions and reassignment within 3 seconds with Tier 3 silent |
| **M2 — dashboard** | Godot and the bridge agree on every field; the rescue chain is visible |
| **M3 — optional strategy** | Filtered directives affect allocation, expire correctly and never block the simulation |
| **M4 — learned-component gate** | A trained component ships only when it beats its baseline; otherwise its fallback ships |
| Rehearsal | Accurate `SHIPPING.md`, an offline run, a backup video and three clean cold boots |

**Perception moved ahead of the auction on purpose.** It sits *below* the blackboard, so the contract boundary means the auction never learns where "victim found" came from — but building the auction against a perfect oracle and re-tuning it later against noisy detections is rework. Do it once, against real data.

### 7.1 Hard cut lines

When reducing scope, cut in this order — top first:
...
An alternative to the language-model advisor was evaluated:
...
The commander experiment is closed; §7.6 records the gate decision.
...
| commander evaluation (3 generated maps, 180 s) | ~86 s each, ~290/hour on 7 workers | commander measurements |
...
Measured in [M-39](MEASUREMENTS.md), demo seed 42 with the evolved roster:
...
> The bar this was measured against — a flat ≥ 75% rescued — **was replaced**
...
**The request:** RL-train the individual units on Kaggle.
**The constraint:** `main` is the rehearsed demo; this work swaps into the demo only if
...
- **The demo plays four maps, 42–45** (§2). Training, checks, bound and gate all
...
Read `rl/log.jsonl` after each training session and stop early if the tripwires fire.
The gate report must distinguish trained, heuristic and routing-only outcomes.
The owner decides whether a measured winner warrants changing and re-rehearsing the demo.

## 8. Risk register

| # | Risk | Likelihood | Impact | Mitigation |
|---|---|---|---|---|
| R1 | 8 GB RAM exhausted with Godot + Python + llama.cpp running together | Medium | Demo-fatal | Budgeted at ~4.7 GB total ([TECHNICAL.md §9](TECHNICAL.md)). Fallback: drop model to Q4_0 or run hivemind at 8 s cadence. |
...
| R3 | GRPO fails to beat base on the gate | **Medium-high** | Low | *This is why the ladder exists.* Retain each gate-approved checkpoint; fall back when an upgrade fails. |
| R4 | Kaggle session dies mid-run (9 h cap, or preemption) | High | Medium | Checkpoint every N steps and at 8 h wall clock; resume via output→input chaining. |
...
| R9 | Integration between sim / nodes / dashboard slips | Medium | High | Frozen contract, integration at M2 and schema round-trip tests. |
| R10 | Live demo fails for non-code reasons | Low | Total | Final backup video and three cold-boot rehearsals. Run with wifi off by design. |
